...
abstract class DB {
		 ...
		 * 向数据库中插入一条记录
		 */
		function insert($array=null, $filter=1, $validate=1){
			if(is_null($array))
				$array=$_POST;

			if($validate){
				$vali=Validate::check($this->tabName, $array, "add", $this);
			}else{
				$vali=true;	
			}
		 ...
		 * 更新数据表中指定条件的记录
		 */
		function update($array=null, $filter=1, $validate=1){
                        if(is_null($array))
				$array=$_POST; 

			if($validate){
				$vali=Validate::check($this->tabName, $array, "mod", $this);
			}else{
				$vali=true;	
			}
